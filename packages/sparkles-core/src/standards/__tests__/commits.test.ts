  ...
  for (const key in commit) {
    ...
    describe(key, () => {
      ...
      it(`validates a valid commit: "${commitItem.emoji} ${commitItem.group}: valid commit"`, () => {
        expect(commitItem.validate(`${commitItem.emoji} ${commitItem.group}: valid commit`)).toBeTruthy()
      });
      ...
      it(`asserts a valid commit: "${commitItem.emoji} ${commitItem.group}: asserted commit"`, () => {
        expect(() => commitItem.asserts(`${commitItem.emoji} ${commitItem.group}: asserted commit`)).not.toThrow()
      })
      ...
      it(`creates a commit message: "${commitItem.emoji} ${commitItem.group}: created commit"`, () => {
        expect(commitItem.create('valid commit')).toBe(`${commitItem.emoji} ${commitItem.group}: valid commit`)
      })
    })
  }
});
